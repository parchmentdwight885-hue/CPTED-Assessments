...
const ownership = JSON.parse(
  readFileSync(new URL('../../.claude/ownership.json', import.meta.url), 'utf-8'),
...
describe('.claude/ownership.json', () => {
...
describe('ownership banners agree with the tier map', () => {
  it.each(bannerablePaths('framework_owned'))(
    'framework_owned %s carries the @claude:framework-owned banner',
    (path) => {
      expect(readSource(path)).toContain('@claude:framework-owned');
    },
  );

  it.each(bannerablePaths('shared'))('shared %s carries the @claude:shared banner', (path) => {
    expect(readSource(path)).toContain('@claude:shared');
  });
});
